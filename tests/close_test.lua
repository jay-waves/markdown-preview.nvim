...
stopped(result, true)
preview.setup({ close_on_stop = false })
result = listen()
preview.stop()
stopped(result, false)
preview.setup({ close_on_stop = true })
result = listen()
vim.api.nvim_exec_autocmds("VimLeavePre", {})
stopped(result, true)
preview.stop() -- repeated cleanup must be harmless
print("PASS: explicit stop, opt-out, restart and VimLeavePre deliver the expected SSE lifecycle")
