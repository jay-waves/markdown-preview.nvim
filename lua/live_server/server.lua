...
-- cfg additionally accepts:
--   on_event(event, decoded_data, request) -- GET /__live/event callback
--   routes[path](request) -> body, status?, headers? -- dynamic GET response
            ...
            sock:read_start(function(err_read, chunk)
                ...
                if inst.token then
                    local function path_needs_auth(p)
                        if p == "/__live/events" or p == "/__live/inject" or p == "/__live/event"
                            or p == "/__live/asset" or inst.routes[p] then
                            return true
                        end
                ...
                elseif path_only == "/__live/event" then
                    local event = qparam("event")
                    if not event or type(inst.on_event) ~= "function" then
                        return http_404(sock, "/__live/event")
                    end
                    local decoded = util.url_decode(qparam("data") or "{}")
                    local event_name = util.url_decode(event)
                    vim.schedule(function()
                        local ok_event, event_err = pcall(inst.on_event, event_name, decoded, req)
                        if not ok_event then
                            vim.notify("preview event callback failed: " .. tostring(event_err), vim.log.levels.ERROR)
                        end
                    end)
                    return send_response(sock, 200, { ["Content-Type"] = "text/plain" }, "ok")
                ...
                local route = inst.routes[path_only]
                if type(route) == "function" then
                    local ok_route, body, status, route_headers = pcall(route, req)
                    if not ok_route then
                        vim.schedule(function()
                            vim.notify("preview route failed: " .. tostring(body), vim.log.levels.ERROR)
                        end)
                        return send_response(sock, 500, { ["Content-Type"] = "text/plain" }, "route failed")
                    end
                    if body == nil then return http_404(sock, path_only) end
                    local response_headers = vim.tbl_extend("force", inst.headers, route_headers or {})
                    return send_response(sock, status or 200, response_headers, body)
                end
